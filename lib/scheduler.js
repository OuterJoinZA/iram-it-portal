...
/**
 * Among `events`, rank ALL Medium/Low IT-ticket bookings as candidates to bump:
 * lowest priority first (bump a Low before a Medium), then earliest start
 * (freeing the earliest possible slot serves "ASAP" — the point of bumping at
 * all). Returns an array (possibly empty) — the caller tries each in order,
 * since the "best" candidate on paper might not actually produce a valid slot
 * (see scheduleForDay).
 */
function pickBumpCandidates(events) {
  ...
  return candidates.map(c => ({ event: c.event, ticketID: c.meta.ticketID, priority: c.meta.priority }));
}

/** Back-compat single-result wrapper around pickBumpCandidates. */
function pickBumpVictim(events) {
  return pickBumpCandidates(events)[0] || null;
}
...
function scheduleForDay(priority, minutes, events, windowStart, windowEnd) {
  ...
  // Try each bumpable candidate in priority order — a candidate only "works"
  // if placing the new ticket at its old start time (for the NEW ticket's own
  // duration, which may be longer than the victim's) both fits before the day
  // ends AND doesn't crash into some other event later that day. The first
  // candidate that produces a genuinely valid slot wins; a candidate that
  // doesn't fit is skipped rather than blindly committed to.
  for (const victim of pickBumpCandidates(events)) {
    const oldSlot = {
      start: victim.event.startWithTimeZone || victim.event.start,
      end:   victim.event.endWithTimeZone   || victim.event.end
    };
    const newStartMs = toMs(oldSlot.start);
    const newEndMs    = newStartMs + minutes * 60000;
    if (newEndMs > winEndMs) continue; // new ticket's own duration runs past close of business

    const collides = events.some(e => {
      if (e === victim.event) return false; // the victim's own slot is being vacated
      const s = toMs(e.startWithTimeZone || e.start), en = toMs(e.endWithTimeZone || e.end);
      return overlaps(newStartMs, newEndMs, s, en);
    });
    if (collides) continue; // some other event sits between the victim's start and the new end
    ...
    const newSlot = { start: oldSlot.start, end: new Date(newEndMs).toISOString() };

    return {
      slot: newSlot,
      bumped: { ticketID: victim.ticketID, priority: victim.priority, eventId: victim.event.id, subject: victim.event.subject, body: victim.event.body, oldSlot }
    };
  }

  return null; // no bumpable candidate produces a valid slot — try tomorrow
}

const BUSINESS_START_HOUR = 6;  const BUSINESS_START_MIN = 30; // 06:30 UTC == 08:30 SAST
const BUSINESS_END_HOUR   = 15; const BUSINESS_END_MIN   = 30; // 15:30 UTC == 17:30 SAST
...
function dayWindow(dateOnlyIso) {
  ...
  return {
    start: `${dateOnlyIso}T${pad(BUSINESS_START_HOUR)}:${pad(BUSINESS_START_MIN)}:00Z`,
    end:   `${dateOnlyIso}T${pad(BUSINESS_END_HOUR)}:${pad(BUSINESS_END_MIN)}:00Z`
  };
}
 ...
 */
function scheduleTicket(priority, minutes, events, fromDateIso, maxDays = 10, now = new Date()) {
  ...
  for (let offset = 0; offset < maxDays; offset++) {
    ...
    let { start: windowStart, end: windowEnd } = dayWindow(dateIso);

    // For "today" specifically, never offer a slot that's already in the
    // past: if business hours are already over, skip today entirely; if
    // we're mid-day, start searching from right now instead of 08:30.
    if (offset === 0) {
      if (nowMs >= toMs(windowEnd)) continue;
      if (nowMs > toMs(windowStart)) windowStart = now.toISOString();
    }
