...
//   repeaters: {
//     "<groupId>": {
//       removed: [ "<itemId>", ... ],                  (baked-in items hidden)
//       added:   [ { text: {...}, links: {...} }, ... ] (admin-added items, each
//                                                         scoped to its own clone —
//                                                         see assets/hr-render.js)
//     }
//   }
//
...
function empty() { return { text: {}, links: {}, blocks: [], repeaters: {} }; }

// Shared by the top-level text/links maps and each added repeat-item's own
// scoped fields — same shape, same rules, just applied at different depths.
function sanitizeTextLinks(text, links) {
  const outText = {}, outLinks = {};
  if (text && typeof text === 'object') {
    for (const [k, v] of Object.entries(text)) {
      const kk = key(k);
      if (kk) outText[kk] = str(v, 20000);
    }
  }
  if (links && typeof links === 'object') {
    for (const [k, v] of Object.entries(links)) {
      ...
      if (link.text || link.href) outLinks[kk] = link;
    }
  }
  return { text: outText, links: outLinks };
}
...
function sanitize(input) {
  ...
  const top = sanitizeTextLinks(c.text, c.links);
  out.text  = top.text;
  out.links = top.links;
  ...
  // bullets) — which baked-in items are hidden, and any admin-added items.
  if (c.repeaters && typeof c.repeaters === 'object') {
    for (const [gk, gv] of Object.entries(c.repeaters)) {
      const gkk = key(gk);
      if (!gkk || !gv || typeof gv !== 'object') continue;
      const removed = Array.isArray(gv.removed) ? gv.removed.map(key).filter(Boolean).slice(0, 200) : [];
      const added   = Array.isArray(gv.added)
        ? gv.added.slice(0, 100).map(a => sanitizeTextLinks(a && a.text, a && a.links))
        : [];
      if (removed.length || added.length) out.repeaters[gkk] = { removed, added };
    }
  }
  return out;
}
