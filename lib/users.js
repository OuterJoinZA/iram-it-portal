...
function publicView(u) {
  return { username: u.username, email: u.email || '', role: u.role, active: u.active !== false, createdAt: u.createdAt };
}
...
async function createUser({ username, password, role, email }) {
  ...
  const user = { username, email, salt, passwordHash, role, active: true, createdAt: new Date().toISOString() };
  users.push(user);
  await saveAll(users);
  return publicView(user);
}

async function updateUser(username, { role, active, password, email }) {
  const users = await loadAll();
